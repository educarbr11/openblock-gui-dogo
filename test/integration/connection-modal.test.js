...
// The tests below require DoGoBlock Link to be unavailable, so we can trigger
...
describe('Hardware extension connection modal', () => {
    ...
    test('Message saying DoGoBlock Link is unavailable (BLE)', async () => {
        await driver.quit();
        driver = getDriver();
        ...
        findByText('DoGoBlock Link'); // DoGoBlock Link is mentioned in the error modal

        const logs = await getLogs();
        await expect(logs).toEqual([]);
    });

    test('Message saying DoGoBlock Link is unavailable (BT)', async () => {
        await loadUri(uri);
